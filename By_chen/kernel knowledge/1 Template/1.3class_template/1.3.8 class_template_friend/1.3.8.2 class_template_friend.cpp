/*
    ...
    1.3 类模版
        ...
        1.3.7 类模版分文件编写
            问题:类模版中成员函数的创建时机是在调用阶段,导致分文件编写时链接不到.
            解决方法:
                1.直接包含.cpp源文件
                2.将声明和实现写到同一个文件夹中,更改后缀为.hpp,.hpp是约定的名称,并不是强制的
        1.3.8 类模版与友元
            掌握类模版配合友元函数的类内和内外实现.
            全局函数类内实现:
                直接在类内声明友元即可
            全局函数类外实现:
                需要提前让编译器知道Person 类的存在,将函数放在前面,让类内知道该代码的实现.
...
// template <class NameType,class AgeType>     //函数声明
// class Person;
...
// }
//1.全局函数类内实现
template <class NameType,class AgeType>         //默认int型
class Person
{
    //1. 全局函数类内实现
    friend void showPerson(Person<NameType,AgeType> &p)     //friend保证访问私有元素
    {
        cout<<"Name:\t"<<p.m_Name<<endl;        
        cout<<"Age:\t"<<p.m_Age<<endl;        
    }
